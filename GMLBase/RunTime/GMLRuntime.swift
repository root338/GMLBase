...
public extension NSObject {
    ...
    static func propertyNames(shouldLoadSuper: (AnyClass) -> Bool = { _ in false } ) -> [String]? {
    ...
    func propertyNames(shouldLoadSuper: (AnyClass) -> Bool = { _ in false } ) -> [String]? {
        return type(of: self).propertyNames(shouldLoadSuper: shouldLoadSuper)
    }
    
    func toDictionary(shouldLoadSuper: (AnyClass) -> Bool = { _ in true }) -> [String : Any]? {
        guard let propertyNames = propertyNames(shouldLoadSuper: shouldLoadSuper) else { return nil }
        var dic = [String : Any]()
