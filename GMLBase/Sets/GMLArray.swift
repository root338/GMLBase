...
/// 扩展数组方法的错误信息
public enum GMLArrayError : Error {
    /// 索引越界
    case index
    /// 类型不支持
...
public extension Array {
    func toJSON(encoding: String.Encoding = String.Encoding.utf8) throws -> String? {
...
public extension Array {
    ...
    /// - Throws: 不支持类型数组报 GMLArrayError.typeNoSupport(message:) 异常
    func index(at obj: Element) throws -> Int? {
        guard let list = self as? [NSObjectProtocol] else {
            throw GMLArrayError.typeNoSupport(message: "请使用 firstIndex(where:) 自定义实现")
        }
    ...
    /// 替换数组中指定对象
    /// - Throws: 返回 index(at:) 方法异常
    @discardableResult
    mutating func replace(_ obj: Element, replaceObj: Element) throws -> Element? {
        
        guard let index = try index(at: obj) else { return nil }
    ...
    @discardableResult
    mutating func replace(_ obj: Element, replaceCollection: [Element]) throws -> [Element]? {
        guard let index = try index(at: obj) else { return nil }
